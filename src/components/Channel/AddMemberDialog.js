...
function AddMemberDialog({ channelId, ...props }) {
  ...
  return (
      <div>
        <Button variant="outlined" color="#007bb2" onClick={handleClickOpen}>Invite Member</Button>
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
          <DialogTitle id="form-dialog-title" style={{ color: currentMode === "dark" ? "white" : "black" }}>Invite User</DialogTitle>
          <form onSubmit={handleSubmit}>
            <DialogContent>
              ...
              >
                {users.map((d, idx) => {
                  return <MenuItem key={d.uId} value={d.uId} style={{ color: currentMode === "dark" ? "white" : "black" }}>{d.nameFirst} {d.nameLast}</MenuItem>;
                })}
              </Select>
            </DialogContent>
            <DialogActions>
