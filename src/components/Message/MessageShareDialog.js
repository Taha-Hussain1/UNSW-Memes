  ...
  return (
      <div>
        ...
        <Dialog open={open} onClose={handleClose} aria-labelledby="form-dialog-title">
          <DialogTitle style={{ color: currentMode === "dark" ? "white" : "black" }} id="form-dialog-title">Share message</DialogTitle>
          <form onSubmit={handleSubmit}>
            <DialogContent>
              <DialogContentText style={{ color: currentMode === "dark" ? "white" : "black" }}>Enter a channel below to share the message to</DialogContentText>
              <Select
              ...
              >
                {channelsShare.map(d => {
                  return <MenuItem key={d.channelId} style={{ color: currentMode === "dark" ? "white" : "black" }} value={`c${d.channelId}`}>{d.name}</MenuItem>;
                })}
                {dmsShare.map(d => {
                  return <MenuItem key={d.dmId} style={{ color: currentMode === "dark" ? "white" : "black" }} value={`d${d.dmId}`}>{d.name}</MenuItem>;
                })}
              </Select>
              <br/>
              <br/>
              <DialogContentText style={{ color: currentMode === "dark" ? "white" : "black" }}>Optionally, enter an additional message</DialogContentText>
              <TextField multiline fullWidth rowsMax={4} value={message} onChange={handleChange} InputProps={{ style: { color: currentMode === "dark" ? "white" : "black" } }}/>
            </DialogContent>
            <DialogActions>
